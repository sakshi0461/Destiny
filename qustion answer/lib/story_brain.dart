...
class Storyline{
  int _questionno=0;
  List<Story> _storytake=[
    Story('Your car as blown a tire on a winding road in the middle of nowhere with no cell phone reception. You decide to hitchhike. A rusty pickup truck rumbles to a stop next to you. A man with a wide brimmed hat with soulless eyes opens the passenger door for you and asks: "Need a ride, boy?".',
  ...
  String getchoice1(){
    return _storytake[_questionno].choice1;
  }

  String getchoice2(){
    return _storytake[_questionno].choice2;
  }

  String getques(){
    return _storytake[_questionno].storytitle;
  }

  void choiceno(int storyno){
      if(_questionno==0){
        if(storyno==1)
        _questionno=2;
        else
      ...
      else if(_questionno==1){
        if(storyno==1)
        _questionno=2;
        else
        _questionno=3;
      }
      else if(_questionno==2){
        if(storyno==1)
        _questionno=5;
        else
        _questionno=4;
    ...
    bool makevisible(){
      if(_questionno==3 || _questionno==4 || _questionno==5){
        return false;
      }
      return true;
    }
